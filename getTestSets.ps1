...
#echo "$orchurl/identity_/connect/token"
$token = Invoke-RestMethod -Method Post -Uri "$orchurl/identity_/connect/token" -Headers $headers -Body $body 
...
$response = Invoke-RestMethod -Method Get -Uri "$orchurl/$org/$tenant/orchestrator_/odata/TestSets?%24filter=startswith(Name%2C%20%27$filter%27)" -Headers $headers
	
foreach($item in $response.value) {
	$response = Invoke-RestMethod -Method Get -Uri "$orchurl/$org/$tenant/orchestrator_/odata/Folders($($item.OrganizationUnitId))" -Headers $headers
    ...
    $result | Add-Member -Type NoteProperty -Name "testsetname" -Value $item.Name
